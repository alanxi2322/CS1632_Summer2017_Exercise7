...
public class MonkeySim {
    ...
    public static int getStartingMonkeyNum(String[] args) {
      ...
      // REFACTORED VERSION OF GETFIRSTMONKEY
      // ELIMINATED LOOP AND SIMPLIFIED ARRAYLIST SEARCH FOR FIRST MONKEY
      ...
      // REFACTORED VERSION OF STRINGIFYRESULTS
      // ELIMINATED NEED TO MAKE NEW STRING WITH EACH CONCATENTATION
      // AND REMOVED UNNECESSARY HEADER SECTION

      public static String stringifyResultsRefactored(int c, Monkey m, Monkey m2) {
      ...
      public static int runSimulation(List<Monkey> ml, MonkeyWatcher mw) {
      	...
      	// while (!getFirstMonkey(ml).hasBanana()) { // ORIGINAL METHOD CALL
        while (!getFirstMonkeyRefactored(ml).hasBanana()) { // REFACTORED METHOD CALL
      	    mw.incrementRounds();
            ...
            int n = nextMonkeyAndResize(m, ml);
      	    ...
      	    m2.throwBananaTo(b);
            // String s = stringifyResults(mw.getRounds(), m, m2); // ORIGINAL METHOD CALL
      	    String s = stringifyResultsRefactored(mw.getRounds(), m, m2); // REFACTORED METHOD CALL
